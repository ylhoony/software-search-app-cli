  ...
  def self.scrape_software(category)
    ...
    doc.css(".listing").each do |element|
      if SoftwareSearch::Software.find_by_name(element.css(".listing-name a").text.strip)
        ...
        software.overall_rating = element.css(".reviews").attribute("data-rating").value.split("/")[0] || 0
        software.reviews = element.css(".reviews").attribute("data-rating").value.split(" - ")[1] || 0
